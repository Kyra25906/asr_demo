...
## 2026-08-14：显示一致性修复——"观察层与执行层"的自洽（AUTO_OK）

- **用户抓出的显示矛盾**：兜底接住"时间为10分钟"后，影子摘要显示"待确认动作=no_action；已执行：已将对问题 1 的答复…"——LLM 原始判断（no_action）与实际执行（answer）并排打印，自相矛盾。
- **根因**：兜底用 `dataclasses.replace` 只替换了 `pending_action`（要执行的动作），而 `clarification_action`/`destination`/`permission` 这些**描述 LLM 原始判断的字段原样保留**。显示函数同时打印两者，于是两层信息打架。
- **设计澄清**：观察摘要本来就有意保留"LLM 怎么说"（诊断价值），执行反馈显示"实际做了什么"。但当兜底**改写了动作**时，显示字段必须跟随实际动作，否则用户读起来是矛盾的——**兜底改变的不只是执行，还有"这段最终被理解成什么"**。
- **顺带修了"仍需补充：空"**：字段补齐但确认（requires_confirmation）未完成时，`missing_fields` 为空、问题仍 unresolved，原逻辑输出"仍需补充：空"。修复为三分支：已解决 / 仍需补充：X / 仍需确认。
- **测试**：新增 `AnswerConfirmationPendingTests`（字段补齐+确认待定 → reason 含"仍需确认"且不含"仍需补充："）。全量 468 项通过。GAPS-02 项 E（显示 bug）结案，待真实复验。
...
## 2026-08-14：体验走查机制——把"体验"从感觉变成可检查的清单（UX-BASELINE-01）

- **用户提出的问题**：agent 每轮验收只聚焦"能力有没有实现"（功能验收），忽略"用户体验如何"；终端输出看不出体验好坏，双方对"最终构建的东西怎么样"说不清楚。
- **根因诊断**：①证据类型错配——REAL_OK 的证据（会话 ID、JSONL 数、token 数）证明"功能正确发生"，不证明"体验好"；②体验是人的判断——agent 只能收集证据，最终裁决权在用户；③输出未分层——main.py 42 处散落 print，用户对话/调试信息/状态混排，实时看不过来。
- **落地机制**：新增 `docs/UX_WALKTHROUGH_CHECKLIST.md`——九维走查表（信息分诊/顺序正确/问题唯一/编号一致/反馈及时/容错可见/可理解性/语音预算/响应速度）+ 固定走查脚本 + 记录模板 + 判定规则（UX_PENDING→UX_WALKED→UX_CONFIRMED/UX_ISSUES）；CLAUDE.md 收尾 6 步改为 7 步（第 6 步体验走查）；任务清单状态定义加 UX 列。
- **基线走查（会话 20260814_174441，用户口述）**：九维中 1/3/4/6/7 为 ✗，2/5/9 为 ✓，8 不适用（TTS 未接）。8 项问题登记 UX-01~08：DEBUG 混排、追问被淹没、无编号回答误判、降级无人话、结束语失败无提示、双句号、编号未分离、汇总含开发语言。原始证据 `results/walkthrough_baseline_session_20260814_174441.txt`。
- **知识点一：功能验收与体验验收是两个维度**。
  - 白话解释：功能验收像"验货"——货有没有、规格对不对；体验验收像"试用"——好不好用、顺不顺手。货验过了不等于好用好。以前我们只验货（测试绿、REAL_OK），把"货有了"当成"好用了"，两边自然说不清。
  - 专业术语：功能正确性（Functional Correctness）与可用性（Usability）分离；客观可测行为（Observable Behavior）vs 主观体验（Subjective Experience）；验收证据类型错配（Evidence Type Mismatch）。
  - 项目实际体现：`docs/UX_WALKTHROUGH_CHECKLIST.md` 明确"REAL_OK ≠ UX_CONFIRMED"，体验最终裁决权在用户，agent 不得代填。
- **知识点二：终端输出是"证据流"，不是"体验面"**。
  - 白话解释：语音助手的输出渠道应该是"说为主、看为辅"——屏幕是事后确认用的，不是口述时盯着的。让用户边说话边看终端，就像开车时让司机边开车边读说明书。
  - 专业术语：交互渠道分工（Channel Separation）；认知负担（Cognitive Load）；输出分层（Layered Output）——VOICE/SCREEN/DEBUG/RECORD 四层（见 OUTPUT_PRESENTATION_POLICY.md）。
  - 项目实际体现：基线走查中用户直接反馈"边口述边判断看不过来"——这本身就是第 1/7 维失败的活证据；走查方式因此改为"口述者不看屏幕、输出重定向文件、判断事后离线做"。
- **知识点三：Python 输出重定向的缓冲陷阱**。
  - 白话解释：print 的内容平时立刻显示在终端，是因为终端是"行缓冲"；一旦重定向到文件就变成"块缓冲"——攒满一坨才写盘。所以后台重定向运行时，文件一直空着，人以为程序没启动，其实在跑。
  - 专业术语：标准输出缓冲模式（Buffering Modes）：行缓冲（line-buffered）vs 全缓冲（block-buffered）；`python -u` 强制无缓冲（unbuffered）。
  - 项目实际体现：本次走查第一次启动用普通 `python -B`，输出文件 SIZE=0；改用 `python -u -B` 后输出即时落盘。
- **知识点四：并行实例会互相污染真实走查**。
  - 白话解释：同一台机器上两个程序同时开着麦克风等同一个唤醒词，用户喊一声"小科小科"，两个都被唤醒、都录音、都调 LLM——两边输出混在一起，证据没法用。
  - 专业术语：测试环境隔离（Test Isolation）；共享资源竞争（Shared Resource Contention）。
  - 项目实际体现：本次走查存在并行实例（`results/walkthrough_baseline_20260814.txt`，已归档），记录教训：下次走查只允许一个实例运行。
- **测试基线**：本轮无代码改动（纯文档+真实走查），体验状态 `UX_ISSUES`，8 项问题待 PRESENT/TTS/前端逐项闭环后复走九维表对比。

## 2026-08-14：嘈杂识别样例入语料——"测试断言与数据规模耦合"的小教训

- **背景**：体验基线走查会话 `20260814_174441`（背景嘈杂）暴露三个真实 ASR 误识别样例：'五毫升→防生'、'（环境噪声）→제가.'、'结束→.别束'。用户指示：**登记进鲁棒性语料，修复走既有 ASR 线**（不混入输出层改动）。
- **改动**：`evaluation/narration_robustness/narration_plan.json` 28→31 段（段 29 `asr_noise_entity` / 段 30 `uncertain_semantics` / 段 31 `asr_truncated`，expected 标注对齐既有段模式，note 注明真实来源）；`tests/test_narration_robustness_plan.py` 三处写死断言同步 28→31、experiment 段 13→14。
- **知识点：测试断言与数据规模耦合**。
  - 白话解释：测试里写死"语料有 28 段"，就像店门口挂的牌子写"本店 28 道菜"——菜单加了 3 道菜，牌子就得跟着改。这不是 bug，是"写死的数量"必然随数据变化失效。
  - 专业术语：Magic Number 断言（写死常量断言）与数据规模耦合（Test-Data Coupling）；维护时必须同步更新（Keep in Sync）。
  - 项目实际体现：`test_narration_robustness_plan.py` 的 `len(plan.segments)==28` 与 `len(experiment_segments)==13` 两处断言随语料扩充失效；语料 schema 本身 21 项一次通过——说明"结构合法性校验"与"数量断言"是两层东西，前者不随数据变化，后者会。
- **测试基线**：全量 468 项通过。UX-10 修复时机由 ASR 任务线（AUDIO-PREROLL 截音/热词/后处理）决定。

## 2026-08-14：迁移对照必须含"体验质量"维度——"动作在≠体验在"（用户指出"新链路没感觉改善/功能丢了"）

- **用户的核心批评（成立）**：①PRESENT 被"修完再做"无限推迟；②真实验收暴露的问题靠用户自己读终端发现，agent 发现不了；③"新链路没感觉改善哪里"，感觉"功能丢了"。
- **根因一：迁移对照只追踪"动作有无"，不追踪"体验质量有无退化"**。5.3 表写"`display_completed_segments → display_shadow_observation` 承接"，但旧链输出用户语言（"已确认第1步"），新链输出开发语言摘要（"目标=权限=已填入['action','object']仍需确认。。"）——动作承接了，体验质量从"人话"跌到"机器话"。自动测试和 REAL_OK 都验证不到这一层。
- **根因二：软问题被无限升级成必修**。用"用户能否完成实验记录并拿到正确数据"重新分类历史真实验收问题：event_store NameError/上下文断链/计数错误/结束语崩溃是硬问题；显示矛盾/双句号/降级无人话/误识别是软问题——**历史一半问题是软问题，本可不阻塞 PRESENT，但都被标必修**，导致"永远过不去"。
- **知识点：动作迁移 ≠ 体验迁移**。
  - 白话解释：搬家时，人搬过去了（动作在），但东西没摆好（体验降级）。旧链的"协调回复"是人话，新链的"观察摘要"是机器话——功能动作在，体验质量降了。
  - 专业术语：功能等价（Functional Equivalence）vs 体验等价（Experiential Equivalence）；回归验收的盲区——自动测试验证"行为符合新合同"，不验证"旧体验未退化"。
  - 项目实际体现：`PROJECT_ARCHITECTURE.md` 5.4 用户可见输出质量对照表；CLAUDE.md 迁移对照约定升级为"逐项标注等价/降级/丢失，agent 不得自行判定功能等价"。
- **知识点：硬问题/软问题的阻塞判据**。
  - 白话解释：真实验收出问题，不能一看到问题就说"必修"。先问一句：用户还能不能把实验记录做完、拿到的数据对不对？能且对=软问题（体验打折，不挡 PRESENT）；不能或错=硬问题（必修）。
  - 专业术语：阻塞判定（Blocking Criterion）；缺陷严重度分级（Severity Triage）——用"核心流程可用性 + 数据完整性"作客观分界，而非主观"看起来严重"。
  - 项目实际体现：任务清单第 1 节"硬问题 vs 软问题"表 + 推进闸门（硬问题修完即进 PRESENT，软问题与 PRESENT 并行）。
- **推进闸门（用户 2026-08-14 最终决策）**：硬问题修完即进 PRESENT，不设固定日期（进展快于计划则提前）；软问题与 PRESENT 并行、出现即修。

## 2026-08-14（夜）：价值叙事纠错 + 四个真发现

- **"两次调用"是偷换概念**：旧路本就是一次 LLM 调用（本地精确规则判命令零 LLM + 一次结构化 LLM），"两次调用"只出现在被否决的独立分类器方案和双轨并行期。头号卖点"省一半调用"不成立。
- **非阻塞录音丢失 + 文案说谎**：删后台线程后 main 同步串行，但 `main.py:320` 仍打印"无需等待 LLM 处理完成"、`main.py:111` 打印"旧流程继续"（旧流程已删）。5.3 表曾把"后台线程"误写成"无此需求"。教训：删代码不改引用旧行为的文案 = 系统说谎；能力丢失写成"无此需求" = 假账。
- **DEFER reversible 被 LLM 弃权**：`intent_policy.py` 第 173 行写死 `evidence == LOCAL_SEMANTIC`，LLM 候选（`LLM_CANDIDATE`）即使 `reversible=True` 也 `DO_NOT_EXECUTE`——自然表达命令落进真空，此前误归因"schema 缺枚举值"。教训：初心是 LLM 兜底，命令侧却堵死 LLM，自相矛盾。
- **无编号兜底缺"紧邻"约束**：`decide_unnumbered_answer` 只查"恰好一问题+短句+字段子集"，不看问题是否紧邻，隔几句后无编号口述会错位。修复：问题来源段+1 == 当前段才自动接。
