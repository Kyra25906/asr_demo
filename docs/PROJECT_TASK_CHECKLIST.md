...
> 执行 `ASR-CMD-02-HOTWORD-01`：继续使用同一批14条WAV，先定义固定热词候选和对照报告；不得覆盖模型原文，也不直接把错词替换成热词。
...
|---:|---|---|---|---|---|
| 1 | `P0` | `ASR-CMD-02-HOTWORD-01` 固定热词参数对照 | `TODO` | 同一批14条WAV比较无热词与固定热词；一次只改热词参数 | 产出可重复报告并检查改善、回退和误触发 |
...
| 4 | `P0` | `ASR-CMD-02` 固定数据比较ASR方案 | `DESIGN` | 固定中文对照已完成，下一候选为热词 | 至少找到一种可重复改善，或用数据证明当前模型限制 |
...
| `ASR-CMD-02-HOTWORD-01` | `P0` | 无热词与固定热词参数对照 | `TODO` | 继续使用同一14条WAV和auto基线；先核对SenseVoice/FunASR当前版本的热词参数支持，必须报告改善与回退，不做原文覆盖式替换 |
...
| 2026-08-09 | 纠正环境误判并完成固定中文参数真实对照 | Python3.11全量218项通过；比较工具8项通过 | `.venv`始终正常，先前为沙箱执行限制；14条WAV结果为auto文本8/14、意图9/14，zh文本8/14、意图10/14，但存在文本回退 | `P0 ASR-CMD-02-HOTWORD-01`固定热词对照 |
