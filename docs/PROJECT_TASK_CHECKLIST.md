...
> 执行 `INTENT-02-LLM-INTEGRATION-01`：用Fake LLMClient把LLMIntentClassifier接入IntentRouter，验证精确绕过、自然低风险、高风险确认、uncertain和异常降级的整链路；不接main和真实网络。
...
|---:|---|---|---|---|---|
| 1 | `P0` | `INTENT-02-LLM-INTEGRATION-01` 分类适配器模块集成 | `TODO` | Fake LLMClient→LLMIntentClassifier→IntentRouter→风险策略 | 五类关键分支整链路通过且精确命令零模型调用 |
...
| `INTENT-02-LLM-INTEGRATION-01` | `P0` | LLM适配器与IntentRouter模块集成 | `TODO` | 先使用Fake LLMClient跑完整链路，确认精确命令不调用模型、候选经过风险策略、异常/uncertain不执行；通过后才做真实DeepSeek |
