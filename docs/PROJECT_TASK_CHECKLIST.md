...
> 执行 `ASR-CMD-02-LANGUAGE-01`：使用既有14条人工标注WAV，先验证当前FunASR版本支持的固定中文参数，再与language=auto做同数据对照；一次只改变语言参数，不要求用户新录音。
...
|---:|---|---|---|---|---|
| 1 | `P0` | `ASR-CMD-02-LANGUAGE-01` 固定中文参数对照 | `TODO` | 既有14条WAV上比较auto与受支持的中文值；固定其他参数 | 产出可重复JSON报告，分别统计文本和意图指标 |
...
| 4 | `P0` | `ASR-CMD-02` 固定数据比较ASR方案 | `TODO` | 同一批WAV依次比较当前配置、固定中文参数、热词或候选方案 | 至少找到一种可重复改善，或用数据证明当前模型限制 |
| 5 | `P0` | `INTENT-01` 自然表达与风险定义 | `TODO` | 定义查看、暂缓、回答、结束等意图的容错边界和风险等级 | 明确哪些可直接执行、哪些要确认、哪些不得由LLM执行 |
| 6 | `P0` | `INTENT-02` 轻量IntentRouter | `TODO` | ASR后先判断控制意图，再决定进入命令模块或实验LLM | 正常、边界、误识别及高风险测试通过 |
| 7 | `P1` | `PRESENT-INTEGRATE-01` 最小消息链路 | `TODO` | 先把一种待确认消息接入PresentationMessage→Presenter | 用户消息、状态日志和开发日志开始分流 |
...
|---|---|---|---|---|
| `ENV-01` | `P1` | 修复或重建主 `.venv` | `REAL_OK` | 恢复官方Python 3.11.9并用venv --upgrade刷新启动器；main导入、118项测试和三类模型加载通过 |
...
| `ASR-CMD-02-LANGUAGE-01` | `P0` | language=auto与固定中文参数对照 | `TODO` | 使用已有14条人工标注WAV，不等待新24条；先核对当前FunASR支持值，再固定其他生成参数比较 |
