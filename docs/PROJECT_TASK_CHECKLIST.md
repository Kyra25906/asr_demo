...
> 在项目 `.venv` 中完成 `PRESENT-01` 与 `COMMAND-01` 全量回归验收；通过后进入 `CLARIFY-09` 问题暂缓生命周期。
...
| ID | 任务 | 状态 | 验收证据/备注 |
|---|---|---|---|
| `ENV-01` | 修复或重建主 `.venv` | `BLOCKED` | `pyvenv.cfg` 指向已不可用的 Python 3.11.9，启动时报 Unable to create process；重建前需确认采用的Python版本 |
| `ENV-02` | 完成备用 `.venv-py314` 的项目依赖和外部程序检查 | `TODO` | Python 3.14可启动，但缺少python-dotenv；FunASR检查ffmpeg时出现WinError 5 |
...
| `CLARIFY-09` | 待确认项支持暂缓、回看和稳定编号 | `CODED` | 新增9项生命周期测试；相关核心49项通过；暂缓问题可被后续字段补全，尚未接main |
...
| `COMMAND-01` | 定义统一 InteractionCommand 与命令解析器 | `CODED` | 暂缓/回看已通过处理器接main；结束/肯定仍沿用旧入口，待后续统一迁移和项目venv全量回归 |
| `CLARIFY-MAIN-01` | 暂缓和回看命令接入main | `CODED` | 原始ASR先保存；存储失败不改状态；54项核心测试与main语法检查通过，待真实口述 |
...
| `PRESENT-02` | 当前回答回执优先于旧后台追问 | `CODED` | main先处理当前暂缓/确认回执，再展示ASR期间完成的后台结果；待真实口述 |
