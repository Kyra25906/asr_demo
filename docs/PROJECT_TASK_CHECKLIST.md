...
| 7 | `P1` | `GAPS-FIX-END-01` 结束语非精准命中 → 追问 → 肯定后结束（用户 2026-08-14 晚改方案一） | `TODO` | LLM 识别到结束意图（非精确"结束实验记录"，如"今天先记录到这里吧"）→ 系统追问"是否结束？"→ 用户肯定（"是/是的"）→ 结束；不再出现"观察失败 ValueError" | 用户决策变更（2026-08-14 晚）：从方案二（固定结束语提示）改回**方案一（追问确认后结束）**。实现点：END_SESSION 的 `REQUEST_CONFIRMATION` 真正闭环——main 检测到 `end_session_confirmation` 目的地时显示追问，下一段 affirm 命中则结束 |
| 8 | `P1` | `GAPS-FIX-DEFER-01` DEFER 可逆候选接上下文校验 + 按编号暂缓真实验收（用户 2026-08-14 晚） | `TODO` | ①`intent_policy.py` 让 reversible 的 `LLM_CANDIDATE` 也走 `REQUIRE_CONTEXT`（不再只 `LOCAL_SEMANTIC`）——DEFER 的 `reversible=True` 真正接上，LLM 识别"我先跳过"不再弃权，交上下文层校验目标后暂缓；②补 `defer_targeted`（按编号暂缓"问题二先跳过"）+ 真实验收 | 根因修正（2026-08-14 晚）：不是"schema 缺枚举值"，是"LLM 候选被中风险弃权"——自然表达命令落进真空。`reversible=True` 本应让可逆操作（DEFER）放行到上下文校验，但代码写死只认 LOCAL_SEMANTIC |
...
| 15 | `P0` | `RESTORE-NONBLOCK-01` 恢复非阻塞录音（评委 2026-08-14 发现：main.py 已无后台线程） | `TODO` | 恢复后台处理（或等价方案），让"说完继续说、连续口述不卡"回来。旧路靠 `SessionProcessingQueue` 后台线程 + 背压 max_pending_tasks=4（真机验收过"连续5段不卡"）；现主循环同步串行（observe 调 LLM 期间麦关闭，热2.6s/冷10.98s），能力**已丢失且此前未登记** | 旧路用户可感知能力，被 5.3 表误写成"无此需求"；与 PRESENT 并行恢复；真实验收"连续口述不卡" |
| 16 | `P1` | `RESTORE-DEGRADED-HINT-01` 恢复降级人话提示（评委 2026-08-14 发现） | `TODO` | LLM 失败/降级时给用户一句人话（"原始记录已保存，结构化处理暂时不可用"），现只有"[统一链]目标=degraded_note…"开发日志，用户会以为系统崩了 | 属功能缺陷而非美观；并入 PRESENT 降级提示项，或先补最小提示 |
| 17 | `P1` | `SYNC-UI-CLAIMS-01` 用户文案与实际行为一致性核查（评委 2026-08-14 发现） | **已扫（发现 2 谎话 + 2 误导）** | 全项目运行时 print 共 94 处，逐条核对"承诺 vs 实际"。**硬谎话 2 处**：①main.py:320"无需等待 LLM 处理完成"（实为同步等待）；②main.py:111"观察失败…旧流程继续"（旧流程已删，无旧流程接手）。**措辞误导 2 处**：③main.py:589"立即继续监听"（同步下是等 LLM 算完）；④main.py:646"提交 M 段实验口述"（内部术语，用户易困惑）。**DEBUG 泄漏**一批（[LLM请求/响应]/[统一链]/状态变化/路径/进度条，已登记 UX-01/UX-11） | 谎话 2 处随 RESTORE-NONBLOCK-01 一并处理（恢复非阻塞则 320 句为真；111 句改为"ASR 原文已保存"）；误导 2 处随 PRESENT 文案统一改 |
| 18 | `P1` | `ANSWER-FALLBACK-ADJACENCY-01` 无编号兜底加"紧邻"约束（用户 2026-08-14 晚指出） | `TODO` | `decide_unnumbered_answer` 只查"恰好一问题+短句+字段子集"，**缺"问题是否紧邻"约束**——隔了几句后无编号口述会被误判成回答（错位风险）。修复：加第 5 条规则，**问题来源段 + 1 == 当前段**才允许自动接，否则要带编号 | 数据错位风险，接近硬问题。代价：问题缺两个字段时，分两句连续补字段，第二句不紧邻、要带编号（用户已默认接受最严格版，如需放宽为"中间无无关实验口述"再议） |
