...
最后更新：2026-08-14（INTENT-02-CLEANUP-FLAGS-01 去标志位 REAL_OK）
...
- 当前全量自动测试：`438 tests OK`（Python 3.11.9，2026-08-14）
- 环境验证：核心依赖和 `src.main` 导入成功；首次沙箱内失败已确认是执行权限误判，不是 `.venv` 损坏
- 最近真实连续口述会话：`20260814_095506`
- 最近真实会话已验证：去标志位不退化——启动打印"统一理解链已启用（唯一默认路径）"；6 段口述（2 实验 + 2 查看 + 1 弃权 + 1 旧门卫查看），结束"提交 2 段实验口述"，控制/弃权不占计数；ASR 误识别"看待确认问题"被统一链正确理解为 review；"还有什么问题？"走精确快速路径零 LLM；上下文计数 2 = 事件数
...
> `MAIN-EVIDENCE-COMMIT-01` AUTO_OK。`MAIN-SESSION-CONTEXT-01` REAL_OK。`MAIN-RUNTIME-HARDEN-01` REAL_OK。`INTENT-02-CLEANUP-FLAGS-01` REAL_OK。当前唯一下一项为 `INTENT-02-CLEANUP-SUBMIT-01`：删旧 submit 相关残留。
