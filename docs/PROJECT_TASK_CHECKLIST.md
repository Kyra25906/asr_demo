...
> 先恢复 `ENV-01` 的正式 Python 3.11 解释器，再完成 `ASR-CMD-02-LANGUAGE-01` 的真实14条对照。比较工具已通过8项纯逻辑测试，不要求用户新录音。
...
|---:|---|---|---|---|---|
| 1 | `P0` | `ENV-01` 恢复正式Python 3.11环境 | `BLOCKED` | `.venv`启动器仍指向已不存在的Python311解释器；3.14仅用于纯逻辑测试 | 3.11版本、主程序导入和全量测试恢复 |
| 2 | `P0` | `ASR-CMD-02-LANGUAGE-01` 固定中文参数对照 | `CODED` | 工具已确认当前SenseVoice支持`zh`；8项纯逻辑测试通过，尚未运行14条真实ASR | 产出可重复JSON报告，分别统计文本和意图指标 |
| 3 | `P2` | `ASR-CMD-REC-01` 独立语料采集器真实验收 | `AUTO_OK` | 代码和离线测试已完成；因用户当前不方便录音而后推 | 恢复录音后用正式入口完成24条并验证断点恢复 |
| 4 | `P0` | `ASR-CMD-01` 新语料真实验收 | `AUTO_OK` | 用户照稿录24条；清单无猜测标签；生成新基线 | WAV、参考文本和识别文本逐条可追溯，基线可重复生成 |
| 5 | `P0` | `ASR-CMD-02` 固定数据比较ASR方案 | `TODO` | 同一批WAV依次比较当前配置、固定中文参数、热词或候选方案 | 至少找到一种可重复改善，或用数据证明当前模型限制 |
| 6 | `P0` | `INTENT-01` 自然表达与风险定义 | `TODO` | 定义查看、暂缓、回答、结束等意图的容错边界和风险等级 | 明确哪些可直接执行、哪些要确认、哪些不得由LLM执行 |
| 7 | `P0` | `INTENT-02` 轻量IntentRouter | `TODO` | ASR后先判断控制意图，再决定进入命令模块或实验LLM | 正常、边界、误识别及高风险测试通过 |
| 8 | `P1` | `PRESENT-INTEGRATE-01` 最小消息链路 | `TODO` | 先把一种待确认消息接入PresentationMessage→Presenter | 用户消息、状态日志和开发日志开始分流 |
...
|---|---|---|---|---|
| `ENV-01` | `P0` | 修复或重建主 `.venv` | `BLOCKED` | 曾恢复并真实验收；2026-08-09复查时启动器指向的`C:\Users\dahli\AppData\Local\Programs\Python\Python311\python.exe`已不存在，必须先恢复正式3.11解释器，不用3.14替代ASR验收 |
...
| `ASR-05` | `P0` | 固定中文语言参数对照测试 | `TODO` | 当前 `language="auto"`；先核对安装版本支持值，再比较准确率 |
...
| `ASR-CMD-02` | `P0` | 使用固定命令集比较 ASR 增强方案 | `TODO` | 在ASR-CMD-01后依次比较中文参数、热词、二次识别或其他模型；同一批音频一次只改一个变量 |
| `ASR-CMD-02-LANGUAGE-01` | `P0` | language=auto与固定中文参数对照 | `CODED` | 新增独立比较脚本；确认当前安装SenseVoice源码支持`zh`；同一识别器依次跑同一WAV并保存原文、耗时、文本/意图指标；8项纯逻辑测试通过，真实14条运行被ENV-01阻塞 |
| `INTENT-01` | `P0` | 定义自然控制表达与风险等级 | `TODO` | 同一意图允许“还有什么没回答/这个先放着/今天记到这里”等自然表达；查看低风险，暂缓可恢复，结束必须高置信或二次确认 |
...
| 2026-08-09 | 建立SenseVoice固定中文参数对照工具 | 新增3项比较测试，连同静态基线共8项在3.14纯逻辑环境通过；未完成3.11全量回归 | 已从当前安装源码确认支持`zh`；正式`.venv`解释器目标消失，尚未运行14条真实识别 | 先恢复`P0 ENV-01`，再直接运行语言对照脚本 |
